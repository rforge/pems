    ...
    source = NULL, pems = NULL) 
{
    ...
    if (!is.null(method)) {
        ...
        if (is.na(ans)) {
            if (if.missing == "stop") {
                rep.2 <- paste("could not unambiguously match fun.method '", 
                  method, "'", sep = "")
                rep.3 <- paste(method.options, sep = "", collapse = ", ")
                rep.3 <- paste("[valid option(s): ", rep.3, "]", 
                  sep = "")
                stop(paste(rep.1, rep.2, rep.3, sep = "\n\t"), 
                  call. = FALSE, domain = NA)
            }
            if (if.missing == "return.false") { return(FALSE) }
            rep.2 <- paste("unmatched method handling option '", 
                if.missing, "' unrecognised", sep = "")
            ...
            if (if.valid == "return.valid") {
                ...
                rep.2 <- paste("valid case method handling option '", 
                  if.valid, "' unrecognised", sep = "")
                stop(paste(rep.1, rep.2, sep = "\n\t"), call. = FALSE, 
                  domain = NA)
            }
        }
        ...
        rep.2 <- paste("NULL case method handling option '", 
            if.null, "' unrecognised", sep = "")
        stop(paste(rep.1, rep.2, sep = "\n\t"), call. = FALSE, 
            domain = NA)
    }
    return(method)
